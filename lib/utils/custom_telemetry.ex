defmodule AtmPseudoAppCa.Utils.CustomTelemetry do
  alias AtmPseudoAppCa.Utils.DataTypeUtils
  import Telemetry.Metrics

  @service_name Application.compile_env!(:atm_pseudo_app_ca, :custom_metrics_prefix_name)
  ...
  def custom_telemetry_events() do
    :telemetry.attach("atm_pseudo_app_ca-plug-stop", [:atm_pseudo_app_ca, :plug, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("atm_pseudo_app_ca-redis-stop", [:redix, :pipeline, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("atm_pseudo_app_ca-vm-memory", [:vm, :memory], &handle_custom_event/4, nil)
  ...
  def handle_custom_event([:atm_pseudo_app_ca, :plug, :stop], measures, metadata, _) do
    :telemetry.execute(
